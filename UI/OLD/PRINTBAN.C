...
unsigned long UserWinProcedure(HWND Window,HMSG Message,long Param1,long Param2)
{
  ...
  switch (Message)
  {
     ...
     case DELETETAILWIN:
            ...
            GetProfileString( ProfileName,"Default", "PrintCut10SX",ss,
                        "5.0");
            stx=atof(ss);

            GetProfileString( ProfileName,"Default", "PrintCut10SY",ss,
                        "5.0");
            sty=atof(ss);

            GetProfileString( ProfileName,"Default", "PrintCut10DX",ss,
                        "5.0");
            mDX=atof(ss);

            GetProfileString( ProfileName,"Default", "PrintCut10DY",ss,
                        "5.0");
            mDY=atof(ss);
...
{
    if (Message==WINDOWINIT||Message==KEYDOWN||Message==KEYSTRING)
    {
        if (Message==WINDOWINIT)
        {
